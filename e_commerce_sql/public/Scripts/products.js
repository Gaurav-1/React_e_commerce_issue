...
function show_products(res){
    ...
    <div class='about'>
        <p class='product-name'>${res.name}</p>
        <p>$${res.price}</p>
    </div>
    <button id='${res.product_id}' class='view-detailsBtn'>View Details</button>`
...
function show_popup(){
    ...
    console.log(this.id);
    let data1 = data.find(item=>item.product_id==this.id);
    console.log(data1);
    document.querySelector('#product-image').setAttribute('src',data1.image)
    document.querySelector('#product-name').innerHTML = data1.name;
    document.querySelector('#product-price').innerHTML = '$ '+data1.price;
    document.querySelector('#product-description').innerHTML = data1.description;
