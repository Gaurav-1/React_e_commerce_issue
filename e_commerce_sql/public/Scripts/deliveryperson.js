...
document.getElementById('search').addEventListener('click',()=>{
    ...
    fetch('/deliveryperson/dashboard',{
        method: 'POST',
        headers:{
    ...
    .then(res=>{
        ...
        if(res.message){
            ...
            res.orders.forEach((ele,key)=>{
                // console.log(key,'---',res.orders.length);
                if(key < res.orders.length-1)
                    show_orders(ele);
                else if(key < res.orders.length && type != 'Arrived')
                        show_orders(ele)
                else{
...
function show_orders(res){
    ...
    div.innerHTML +=`<div class='buttons'>
        <button id='${res.order_id}' class='approveBtn btn'>Delivered</button>
...
function approve_order(){
    ...
    console.log(ob);
    fetch('/deliveryperson/approve_order',{
        method: 'POST',
